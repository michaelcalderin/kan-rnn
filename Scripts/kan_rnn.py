...
class EfficientKANRNNCell(nn.Module):
    """
    h_t = tanh(KAN_x(x_t) + KAN_h(h_{t-1}))

    KAN_x and KAN_h already have internal SiLU residuals, so the outer
    tanh acts as the inter-pathway nonlinearity
    """
    def __init__(self, input_size: int, hidden_size: int,
                 grid_size: int = 5, spline_order: int = 3,
                 scale_noise: float = 0.1, scale_base: float = 1.0,
                 scale_spline: float = 1.0, base_activation=nn.SiLU,
                 grid_eps: float = 0.02, grid_range: list = [-1, 1]):
        super().__init__()
        ...
        self.kan_x = KANLinear(input_size,  hidden_size, **kan_kwargs)
        self.kan_h = KANLinear(hidden_size, hidden_size, **kan_kwargs)

    def forward(self, x_t: torch.Tensor, h_prev: torch.Tensor) -> torch.Tensor:
        return torch.tanh(self.kan_x(x_t) + self.kan_h(h_prev))


class EfficientKANRNNLayer(nn.Module):
    """
    Unrolls an EfficientKANRNNCell over a padded sequence and returns
    ALL per-step hidden states (batch, seq, hidden) plus the final valid
    hidden (batch, hidden)
    """
    def __init__(self, input_size: int, hidden_size: int, **cell_kwargs):
        super().__init__()
        self.hidden_size = hidden_size
        self.cell = EfficientKANRNNCell(input_size, hidden_size, **cell_kwargs)

    def forward(
        self, x: torch.Tensor, lengths: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        batch, max_seq, _ = x.shape
        ...
        h_seq  = torch.stack(h_steps, dim=1)

        # Gather the hidden state at each sequence's own last valid step
        idx    = (lengths - 1).clamp(min=0).view(batch, 1, 1)
        idx    = idx.expand(batch, 1, self.hidden_size)
        h_last = h_seq.gather(dim=1, index=idx).squeeze(1)

        return h_seq, h_last


class SentimentEfficientKANRNN(nn.Module):
    """
    Stacked EfficientKAN-RNN for sequence classification
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        for i in range(num_layers):
            in_size = embed_dim if i == 0 else hidden_dim
            self.layers.append(EfficientKANRNNLayer(in_size, hidden_dim, **cell_kwargs))

        self.classifier = KANLinear(hidden_dim, num_classes, **cell_kwargs)

    def forward(self, x: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        """
        x: (batch, seq_len) token ids, sorted by length descending
        lengths: (batch,)
        logits: (batch, num_classes)
        """
        h_seq = self.drop(self.embedding(x))

        for layer in self.layers:
            h_seq, h_last = layer(h_seq, lengths)
            h_seq = self.drop(h_seq)

        return self.classifier(self.drop(h_last))
...
def parse_args():
    ...
    p.add_argument("--weight_decay", type=float, default=0.0, help="L2 weight decay (regularization)")
    return p.parse_args()
...
def main():
    ...
    model = SentimentEfficientKANRNN(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save(model.state_dict(), "kan_rnn_model.pt")
    ...
    plt.tight_layout()
    plt.savefig('kan_rnn_training_curves.png')
    plt.close()
    ...
    plt.savefig('kan_rnn_confusion_matrix.png')
    plt.close()
